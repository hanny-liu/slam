...
// global variables
string first_file = "/home/liuhongwei/workspace/slam/PA7/【作业】特征点法视觉里程计/code/1.png";
string second_file = "/home/liuhongwei/workspace/slam/PA7/【作业】特征点法视觉里程计/code/2.png";
...
// compute the angle
void computeAngle(const cv::Mat &image, vector<cv::KeyPoint> &keypoints) {
    int half_patch_size = 8;//每个关键点在16x16的矩阵内，计算其质心，并计算它与矩阵的几何中心的方向
    ...
    {
        for(auto i=x-8;i<=x+7;i++)
            for(auto j=y-8;j<=y+7;j++)
            {
                m10+=image.at<uchar>(j,i)*(i-x);
                m01+=image.at<uchar>(j,i)*(j-y);

            }
        kp.angle = (atan2(m01,m10))*180/pi; // compute kp.angle
